...
export default function App() {
  ...
  const menuItems = [
    ...
    { icon: '⚙️', label: 'Settings', action: () => { setMenuOpen(false); alert('Settings — coming in next build') } },
    { icon: '🚪', label: 'Sign Out', action: () => { setMenuOpen(false); alert('Sign out — auth coming in next build') } },
  ]
  ...
  return (
    <div className="min-h-screen bg-app flex flex-col" style={{ '--school-color': teacher.schoolColor }}>
      ...
      {/* ── Header ─────────────────────────────────────────── */}
      <header className="sticky top-0 border-b border-elevated" style={{ background: '#0c0e14ee', backdropFilter: 'blur(12px)', zIndex: 50 }}>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="relative">
              ...
              >
                ...
                <span className="text-text-muted text-xs ml-1">{menuOpen ? '▴' : '▾'}</span>
              </button>

              {menuOpen && (
                <>
                  {/*
                    Overlay: z-[80] — above header (z-50) so it covers content behind menu,
                    but the menu dropdown is z-[90] so it sits on top of this overlay.
                    Clicking anywhere on this overlay closes the menu WITHOUT navigating.
                  */}
                  <div
                  ...
                  <div
                    ...
                    style={{ background: '#161923', zIndex: 90 }}
                  >
                    {menuItems.map(item => (
                      <button
                        key={item.label}
      ...
      <nav
        ...
        style={{ background: '#0c0e14f0', backdropFilter: 'blur(12px)', zIndex: 50 }}
      >
        <div className="max-w-6xl mx-auto flex">
          {navItems.map(item => (
            <button
              ...
              style={{ color: activeNav === item.id ? 'var(--school-color)' : '#6b7494' }}
            >
