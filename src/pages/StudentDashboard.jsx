...
function Header() {
  return (
    <div style={{ background: 'linear-gradient(135deg, #ea580c 0%, #db2777 100%)', padding: '18px 16px 20px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <div>
          ...
          <div style={{ fontSize: 9, color: 'rgba(255,255,255,0.4)' }}>Hold widget or tap ✏ to customize · Saved to account</div>
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6 }}>
          <div style={{ position: 'relative' }}>
            ...
            <div style={{ position: 'absolute', top: -4, right: -4, width: 16, height: 16, borderRadius: '50%', background: C.red, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 8, fontWeight: 700, color: '#fff' }}>4</div>
          </div>
...
// ── SW1: Daily Overview (gradient tile — always open, no collapse chrome) ─────
  ...
  return (
    <Widget title="Today's Lessons 📖" borderColor="#1a3a2a">
      <div style={{ background: C.lGrad, borderRadius: 12, padding: '12px 14px' }}>
        ...
        <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.65)', marginBottom: 10 }}>Math · Pages 84–91 · Ms. Johnson · Based on teacher plan</div>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div style={{ background: 'rgba(59,126,244,0.18)', borderRadius: 9, padding: '4px 12px', fontSize: 10, color: C.blue, fontWeight: 700 }}>
            View Worksheet 📄
          </div>
...
function SW3_MyClasses() {
  const classes = [
    { name: 'Math',    teacher: 'Ms. Johnson', gpa: '95.0', gpaSuffix: 'GPA ⭐', suffixColor: C.green, border: C.blue   },
    { name: 'Reading', teacher: 'Ms. Johnson', gpa: '82.0', gpaSuffix: 'GPA',    suffixColor: C.amber, border: C.purple },
    { name: 'Science', teacher: 'Ms. Johnson', gpa: '61.0', gpaSuffix: 'GPA ⚑',  suffixColor: C.red,   border: C.teal   },
    { name: 'Writing', teacher: 'Ms. Johnson', gpa: '88.0', gpaSuffix: 'GPA',    suffixColor: C.green, border: C.pink   },
  ]
  return (
    <Widget title="My Classes">
        ...
        {classes.map(c => (
          <div key={c.name} style={{ background: C.inner, borderRadius: 14, padding: '10px 12px', borderLeft: `4px solid ${c.border}` }}>
            ...
            <div style={{ display: 'flex', alignItems: 'baseline', gap: 6 }}>
              <span style={{ fontSize: 20, fontWeight: 800, color: C.text }}>{c.gpa}</span>
...
function SW6_ClassFeed() {
  return (
    <Widget title="Class Feed 📢">
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div style={{ fontSize: 9, color: C.muted }}>Public links need approval · direct msgs unrestricted</div>
...
function SW7_AIStudyTips() {
  return (
    <Widget title="✨ AI Study Tips" borderColor="#3b2a5a">
      <div style={{ background: '#120d1e', borderRadius: 12, padding: '10px 12px' }}>
        ...
        <div style={{ fontSize: 10, color: '#b090d0', marginBottom: 8 }}>10 min flashcards tonight · same strategy that boosted Reading +8pts</div>
...
function SW8_UploadAssignment() {
  return (
    <Widget title="Upload Assignment 📤" borderColor="rgba(34,201,122,0.2)">
