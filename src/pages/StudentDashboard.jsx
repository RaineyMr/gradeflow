...
function StickyHeader({ name, grade }) {
  const now  = new Date()
    ...
    <div style={{ position:'sticky', top:0, zIndex:100, background:'linear-gradient(135deg, var(--school-color) 0%, var(--school-surface,#000d1f) 100%)', padding:'14px 16px', borderBottom:'1px solid rgba(255,255,255,0.08)' }}>
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between' }}>
        <div>
          ...
          <div style={{ fontSize:17, fontWeight:800, color:'#fff', lineHeight:1.2 }}>{greeting}, {name} 👋</div>
        </div>
        <div style={{ fontSize:11, color:'rgba(255,255,255,0.5)', fontWeight:600 }}>{timeStr}</div>
      </div>
      <div style={{ fontSize:11, color:'rgba(255,255,255,0.45)', marginTop:4 }}>
        {now.toLocaleDateString('en-US',{ weekday:'long', month:'long', day:'numeric' })} · {grade}
      </div>
    </div>
  )
}
...
function VoiceTutorModal({ onClose }) {
  ...
  useEffect(()=>{ return ()=>{ wsRef.current?.close(); if(audioCtxRef.current?.state!=='closed') audioCtxRef.current?.close() } },[])
  ...
  async function startSession() {
    ...
    try {
      ...
      processor.onaudioprocess = (e)=>{
        ...
        ws.send(JSON.stringify({ user_audio_chunk: btoa(String.fromCharCode(...new Uint8Array(int16.buffer))) }))
      }
      ws.onopen = ()=>{ setStatus('active'); statusRef.current='active' }
      ws.onmessage = (event)=>{
        try {
          const msg=JSON.parse(event.data)
          if(msg.type==='audio'&&msg.audio_event?.audio_base64) playAudioChunk(msg.audio_event.audio_base64,ctx)
          if(msg.type==='transcript'&&msg.transcript_event){
            const {speaker_type,text}=msg.transcript_event
            if(text?.trim()) setTranscript(prev=>[...prev,{ role:speaker_type==='user'?'Marcus':'Spark', text:text.trim(), time:new Date().toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'}) }])
          }
        } catch {}
      }
      ws.onerror = ()=>{ setStatus('error'); statusRef.current='error'; setErrorMsg('Connection lost. Please try again.') }
      ws.onclose = ()=>{ processor.disconnect(); source.disconnect(); stream.getTracks().forEach(t=>t.stop()); if(statusRef.current==='active'){ setStatus('ended'); statusRef.current='ended' } }
    } catch(err){ setStatus('error'); statusRef.current='error'; setErrorMsg(err.message||'Could not start session.') }
  }

  function endSession() {
    statusRef.current='ended'; wsRef.current?.close()
    if(audioCtxRef.current?.state!=='closed') audioCtxRef.current?.close()
    setStatus('ended')
    setTranscript(current=>{ if(current.length>0){ const session={ id:Date.now(), date:new Date().toLocaleDateString('en-US',{month:'short',day:'numeric',year:'numeric'}), time:new Date().toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'}), turns:current }; saveTranscript(session); setHistory(loadTranscripts()) }; return current })
  }
    ...
    <ModalShell onClose={onClose}>
      ...
      {history.length===0 ? <div style={{ textAlign:'center', padding:'32px 0', color:T.muted }}><div style={{ fontSize:28, marginBottom:8 }}>💬</div><div style={{ fontSize:13 }}>No sessions yet</div></div> : history.map(session=>(
        <div key={session.id} style={{ background:T.inner, borderRadius:14, padding:14, marginBottom:10 }}>
          ...
          {session.turns.map((turn,i)=>(<div key={i} style={{ marginBottom:6 }}><span style={{ fontSize:10, fontWeight:700, color:turn.role==='Marcus'?T.teal:T.purple, marginRight:6 }}>{turn.role}:</span><span style={{ fontSize:12, color:T.text, lineHeight:1.5 }}>{turn.text}</span></div>))}
        </div>
      ))}
    </ModalShell>
  )
  ...
  return (
    <ModalShell onClose={onClose}>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:16 }}>
        <div><div style={{ fontSize:15, fontWeight:800, color:T.text }}>🎤 Study Tutor</div><div style={{ fontSize:10, color:T.muted }}>Spark · AI Voice Tutor</div></div>
      ...
      <div style={{ textAlign:'center', padding:'16px 0 20px' }}>
        {status==='idle'&&<><div style={{ fontSize:48, marginBottom:12 }}>🎤</div><div style={{ fontSize:13, color:T.muted, marginBottom:20 }}>Talk to Spark about Math, Reading, Science, or Writing</div><button onClick={startSession} style={{ background:`linear-gradient(135deg,${T.purple},#6b3fd4)`, border:'none', borderRadius:16, padding:'14px 32px', color:'#fff', fontSize:14, fontWeight:800, cursor:'pointer', boxShadow:`0 4px 20px ${T.purple}40` }}>Start Session</button></>}
        {status==='connecting'&&<><div style={{ fontSize:36, marginBottom:12 }}>⏳</div><div style={{ fontSize:13, color:T.muted }}>Connecting to Spark...</div></>}
        {status==='active'&&<><div style={{ width:72, height:72, borderRadius:'50%', background:`${T.purple}30`, border:`2px solid ${T.purple}`, display:'flex', alignItems:'center', justifyContent:'center', fontSize:32, margin:'0 auto 12px', boxShadow:`0 0 24px ${T.purple}50` }}>🎤</div><div style={{ fontSize:12, color:T.green, fontWeight:700, marginBottom:16 }}>● Live — Spark is listening</div><button onClick={endSession} style={{ background:`${T.red}22`, border:`1px solid ${T.red}50`, borderRadius:12, padding:'10px 24px', color:T.red, fontSize:13, fontWeight:700, cursor:'pointer' }}>End Session</button></>}
        {status==='ended'&&<><div style={{ fontSize:36, marginBottom:8 }}>✅</div><div style={{ fontSize:13, color:T.green, fontWeight:700, marginBottom:4 }}>Session saved!</div><div style={{ fontSize:11, color:T.muted, marginBottom:20 }}>Find it in Past Sessions anytime</div><button onClick={startSession} style={{ background:`${T.purple}22`, border:`1px solid ${T.purple}40`, borderRadius:12, padding:'10px 24px', color:T.purple, fontSize:13, fontWeight:700, cursor:'pointer' }}>New Session</button></>}
        {status==='error'&&<><div style={{ fontSize:36, marginBottom:8 }}>⚠️</div><div style={{ fontSize:12, color:T.red, marginBottom:16 }}>{errorMsg}</div><button onClick={startSession} style={{ background:`${T.purple}22`, border:`1px solid ${T.purple}40`, borderRadius:12, padding:'10px 24px', color:T.purple, fontSize:13, fontWeight:700, cursor:'pointer' }}>Try Again</button></>}
      </div>
      {transcript.length>0&&(<div style={{ maxHeight:200, overflowY:'auto', background:T.inner, borderRadius:14, padding:12 }}><div style={{ fontSize:10, fontWeight:700, color:T.muted, letterSpacing:'0.06em', textTransform:'uppercase', marginBottom:8 }}>Live Transcript</div>{transcript.map((turn,i)=>(<div key={i} style={{ marginBottom:8 }}><span style={{ fontSize:10, fontWeight:700, color:turn.role==='Marcus'?T.teal:T.purple, marginRight:6 }}>{turn.role}:</span><span style={{ fontSize:12, color:T.text, lineHeight:1.5 }}>{turn.text}</span><span style={{ fontSize:9, color:T.muted, marginLeft:6 }}>{turn.time}</span></div>))}<div ref={scrollRef}/></div>)}
    </ModalShell>
  )
}
  ...
  return (
    <SubPage>
      <div style={{ background:T.header, padding:'16px 16px 20px' }}>
        <button onClick={onBack} style={{ background:T.inner, border:'none', borderRadius:10, padding:'7px 14px', color:T.text, cursor:'pointer', fontSize:13, fontWeight:600, marginBottom:12 }}>← Back</button>
        <h1 style={{ fontSize:20, fontWeight:800, color:'#fff', margin:0 }}>📚 My Classes</h1>
      </div>
      <div style={{ padding:'16px' }}>
        {STUDENT.classes.map(c=>(<div key={c.id} style={{ background:T.card, border:`1px solid ${T.border}`, borderLeft:`4px solid ${c.color}`, borderRadius:16, padding:'14px 16px', marginBottom:10 }}><div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:8 }}><div><div style={{ fontWeight:700, fontSize:14, color:T.text }}>{c.subject}</div><div style={{ fontSize:11, color:T.muted }}>{c.teacher} · {c.period} Period</div></div><div style={{ textAlign:'right' }}><div style={{ fontSize:20, fontWeight:900, color:gradeColor(c.grade) }}>{c.grade}%</div><div style={{ fontSize:11, fontWeight:700, color:gradeColor(c.grade) }}>{c.letter}</div></div></div><GradeBar score={c.grade}/></div>))}
...
function AlertsPage({ onBack }) {
  return (
    <SubPage>
      ...
      <div style={{ padding:'16px' }}>
        {STUDENT.alerts.map(a=>(<div key={a.id} style={{ background:T.card, border:`1px solid ${a.color}30`, borderLeft:`4px solid ${a.color}`, borderRadius:14, padding:'14px 16px', marginBottom:10 }}><div style={{ fontSize:20, marginBottom:6 }}>{a.icon}</div><div style={{ fontSize:13, color:T.text }}>{a.msg}</div></div>))}
      </div>
    </SubPage>
  )
}
...
function HomePage({ navigate }) {
  ...
  const overviewTiles = [
    { icon:'📊', val:STUDENT.gpa, label:'GPA', page:'classes', color:T.secondary },
  ...
  return (
    <div style={{ padding:'12px 12px 0' }}>
      ...
      {show('alerts')&&STUDENT.alerts.length>0&&wrap('alerts',<Widget onClick={()=>navigate('alerts')} style={{ border:`1px solid ${T.red}25` }}><div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:10 }}><div style={{ fontSize:13, fontWeight:700 }}>🔔 Alerts</div><span style={{ background:`${T.red}18`, color:T.red, fontSize:10, fontWeight:700, padding:'3px 10px', borderRadius:999 }}>{STUDENT.alerts.length}</span></div>{STUDENT.alerts.map(a=>(<div key={a.id} style={{ background:T.inner, borderLeft:`3px solid ${a.color}`, borderRadius:10, padding:'9px 12px', marginBottom:6, display:'flex', gap:8, alignItems:'flex-start' }}><span>{a.icon}</span><span style={{ fontSize:12, color:T.text }}>{a.msg}</span></div>))}</Widget>)}

      {show('classes')&&wrap('classes',<Widget onClick={()=>navigate('classes')}><div style={{ fontSize:13, fontWeight:700, marginBottom:12 }}>📚 My Classes</div><div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:8 }}>{STUDENT.classes.map(c=>(<button key={c.id} onClick={e=>{ e.stopPropagation(); navigate('classes') }} style={{ background:T.inner, borderLeft:`3px solid ${c.color}`, borderRadius:12, padding:'10px 12px', border:'none', cursor:'pointer', textAlign:'left' }}><div style={{ fontWeight:700, fontSize:12, color:T.text, marginBottom:2 }}>{c.subject}</div><div style={{ fontSize:10, color:T.muted, marginBottom:6 }}>{c.teacher}</div><div style={{ fontSize:20, fontWeight:800, color:gradeColor(c.grade) }}>{c.grade}%</div></button>))}</div></Widget>)}

      {show('messages')&&wrap('messages',<Widget onClick={()=>navigate('messages')}><div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:10 }}><div style={{ fontSize:13, fontWeight:700 }}>💬 Messages</div><Btn label="+ New" color={T.secondary} onClick={()=>navigate('messages')}/></div>{STUDENT.threads.filter(t=>t.unread).slice(0,2).map(t=>(<div key={t.id} style={{ background:T.inner, borderRadius:12, padding:'10px 12px', marginBottom:8, display:'flex', alignItems:'center', gap:10 }}><span style={{ fontSize:20 }}>{t.avatar}</span><div style={{ flex:1, minWidth:0 }}><div style={{ fontWeight:700, fontSize:12, color:T.text }}>{t.from}</div><div style={{ fontSize:10, color:T.muted, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{t.messages[t.messages.length-1]?.text}</div></div><div style={{ width:8, height:8, borderRadius:'50%', background:T.red, flexShrink:0 }}/></div>))}{!STUDENT.threads.some(t=>t.unread)&&<div style={{ fontSize:11, color:T.muted, textAlign:'center', padding:'8px 0' }}>No new messages</div>}</Widget>)}

      {show('feed')&&wrap('feed',<Widget onClick={()=>navigate('feed')}><div style={{ fontSize:13, fontWeight:700, marginBottom:10 }}>📢 Class Feed</div>{STUDENT.feed.slice(0,1).map(p=>(<div key={p.id} style={{ background:T.inner, borderRadius:12, padding:'10px 12px' }}><div style={{ fontSize:11, fontWeight:600, color:T.secondary, marginBottom:4 }}>{p.author}</div><div style={{ fontSize:12, color:T.text, lineHeight:1.5 }}>{p.content}</div><div style={{ fontSize:10, color:T.muted, marginTop:6 }}>{p.time}</div></div>))}</Widget>)}

      {show('aiTips')&&wrap('aiTips',<Widget style={{ background:'linear-gradient(135deg,#0d1a3a 0%,#000d1f 100%)', border:`1px solid ${T.purple}30` }}><div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:6 }}><div style={{ fontSize:11, fontWeight:700, color:T.purple }}>✨ AI Study Tips</div><button onClick={e=>{ e.stopPropagation(); setVoiceOpen(true) }} title="Start voice session with Spark" style={{ background:`${T.purple}25`, border:`1px solid ${T.purple}50`, borderRadius:8, width:30, height:30, display:'flex', alignItems:'center', justifyContent:'center', cursor:'pointer', fontSize:15, padding:0, flexShrink:0 }}>🎤</button></div><div style={{ fontSize:13, fontWeight:700, color:T.text, marginBottom:4 }}>Science needs your focus!</div><div style={{ fontSize:11, color:T.muted }}>10 min flashcards tonight · same strategy that boosted Reading +8pts</div></Widget>)}

      {voiceOpen&&<VoiceTutorModal onClose={()=>setVoiceOpen(false)}/>}
...
export default function StudentDashboard({ currentUser }) {
  ...
  if(subPage==='classes')  return shell(<SubPage><ClassesPage   onBack={goHome}/></SubPage>)
