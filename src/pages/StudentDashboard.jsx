...
function HomePage({ navigate }) {
  ...
  // ── Student Daily Overview: GPA · Messages · Assignments · Alerts ────────
  // No Scan tile — scan lives in bottom nav only
  const overviewTiles = [
...
export default function StudentDashboard({ currentUser }) {
  ...
  if (page==='classes')  return <><GradesPage   onBack={goHome}/><SharedBottomNav role="student" active="classes"  onSelect={navigate}/></>
  if (page==='grades')   return <><GradesPage   onBack={goHome}/><SharedBottomNav role="student" active="classes"  onSelect={navigate}/></>
  if (page==='messages') return <><MessagesPage onBack={goHome}/><SharedBottomNav role="student" active="messages" onSelect={navigate}/></>
  if (page==='alerts')   return <><AlertsPage   onBack={goHome}/><SharedBottomNav role="student" active="alerts"   onSelect={navigate}/></>
  if (page==='widgets')  return <><Widgets      onBack={goHome}/><SharedBottomNav role="student" active={activeNav} onSelect={navigate}/></>
  ...
  return (
    <div style={{ minHeight:'100vh', background:T.bg, color:T.text, fontFamily:"'DM Sans','Helvetica Neue',sans-serif", paddingBottom:90 }}>
      ...
      {/* Sticky header — greeting only, no camera (camera is in App.jsx top bar) */}
      <div style={{ position:'sticky', top:0, zIndex:100, background:T.header, padding:'14px 16px', borderBottom:'1px solid rgba(255,255,255,0.08)' }}>
        ...
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
          ...
          {/* Unread badge only — no duplicate camera/messages buttons */}
      ...
      <SharedBottomNav role="student" active={activeNav} onSelect={navigate}/>
    </div>
  )
}
