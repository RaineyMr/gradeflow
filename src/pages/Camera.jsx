...
export default function Camera() {
  ...
  useEffect(() => {
    ...
    if (p !== undefined) {
      p.then(() => setCameraReady(true)).catch(() => {
        video.muted = true
        video.play().then(() => setCameraReady(true)).catch(err => {
          setCameraError('Could not start camera: ' + err.message)
          stopStream(); setMode('menu')
        })
      })
    } else {
  ...
  async function openCamera() {
    ...
    if (!navigator.mediaDevices?.getUserMedia) {
      setCameraError(location.protocol === 'http:' && location.hostname !== 'localhost'
        ? 'Camera requires HTTPS. Use the Vercel URL (https://…)'
  ...
  function processImage(dataUrl, base64, mime) {
    setCapturedImage(dataUrl); setCapturedBase64(base64)
  ...
  if (mode === 'processing') return (
    <div className="flex flex-col items-center justify-center py-16 text-center">
      ...
      <p className="text-text-muted text-sm">Detecting score format · Calculating grade...</p>
    </div>
  )

  if (mode === 'camera') return (
    <div>
      <button onClick={() => { stopStream(); setMode('menu') }} className="flex items-center gap-2 text-text-muted text-sm mb-4 hover:text-text-primary">✕ Cancel</button>
      ...
      <button onClick={capturePhoto} disabled={!cameraReady}
        ...
        {cameraReady ? '📸 Capture & Scan' : 'Starting camera...'}
      </button>
    </div>
  )
  ...
  if (mode === 'review') {
    ...
    return (
      <div>
        <button onClick={resetAll} className="flex items-center gap-2 text-text-muted text-sm mb-4 hover:text-text-primary">← Scan Again</button>
        ...
        {sr && !scanError && (
          <div className="p-4 rounded-card mb-4" style={{ background: '#0d1520', border: '1px solid #3b7ef430' }}>
            <div className="flex items-center gap-2 mb-3">
              <span className="text-xs px-2 py-0.5 rounded-pill font-bold" style={{ background: '#9b6ef520', color: '#9b6ef5' }}>🤖 Claude Vision</span>
            ...
            <div className="grid grid-cols-3 gap-2 mb-2">
              <div className="p-2 rounded-card text-center" style={{ background: '#161923' }}>
                ...
                <p className="font-display font-bold text-lg text-white">{sr.earnedPoints ?? '—'}</p>
              </div>
              <div className="p-2 rounded-card text-center" style={{ background: '#161923' }}>
                <p className="tag-label mb-1">Out of</p>
                <p className="font-display font-bold text-lg text-white">{sr.totalPoints ?? '—'}</p>
              </div>
              <div className="p-2 rounded-card text-center" style={{ background: pct >= 70 ? '#0f2a1a' : '#1c1012' }}>
                <p className="tag-label mb-1">Grade</p>
                <p className="font-display font-bold text-lg" style={{ color: scoreColor }}>{pct != null ? `${pct}%` : '—'}</p>
              </div>
            </div>
            {sr.studentName && <p className="text-xs text-text-muted mb-1">👤 <span className="text-text-primary font-medium">{sr.studentName}</span></p>}
            ...
            {sr.teacherComments && <p className="text-xs text-text-muted mt-1">📝 "{sr.teacherComments}"</p>}
            {sr.issues && <p className="mt-2 text-xs" style={{ color: '#f5a623' }}>⚠ {sr.issues}</p>}
          </div>
        )}

        {scanError && (
          <div className="p-3 rounded-card mb-4" style={{ background: '#1c1012', border: '1px solid #f04a4a30' }}>
            <p className="font-semibold text-sm mb-1" style={{ color: '#f04a4a' }}>⚠ Couldn't read automatically</p>
        ...
        <div className="p-4 rounded-card mb-4" style={{ background: '#161923', border: '1px solid #2a2f42' }}>
          ...
          <div className="flex items-center gap-3 mb-3">
            ...
            <div className="flex-1 mt-5">
              <div className="p-2 rounded-card text-center" style={{ background: pct != null ? (pct >= 70 ? '#0f2a1a' : '#1c1012') : '#1e2231' }}>
                <p className="font-display font-bold text-xl" style={{ color: pct != null ? scoreColor : '#6b7494' }}>{pct != null ? `${pct}%` : '—'}</p>
          ...
          <div className="flex gap-2 mb-3">
            {['100', '50', '25', '20', '10'].map(t => (
              <button key={t} onClick={() => setManualTotal(t)}
                className="flex-1 py-1 rounded-pill text-xs font-semibold transition-all"
                style={{ background: manualTotal === t ? 'var(--school-color)' : '#1e2231', color: manualTotal === t ? 'white' : '#6b7494' }}>
                /{t}
              </button>
            ))}
          </div>
          <div className="p-2 rounded-card text-xs text-text-muted" style={{ background: '#0d1520' }}>
            ...
            If teacher wrote −8 on a 100pt test, enter 92/100. If 17/20, enter 17/20 = 85%.
          </div>
        </div>
        ...
        <div className="space-y-4">
          ...
          {pct == null && (
            <p className="text-center text-xs mb-2" style={{ color: '#f5a623' }}>
              ⚠ Enter points earned and total above to calculate grade
            </p>
          )}
          <button onClick={handlePost} disabled={pct == null}
            ...
            Post {pct != null ? `${pct}% (${letter})` : '— enter score above'} to Gradebook →
          </button>
        </div>
      </div>
    )
  ...
  return (
    <div>
      ...
      <input ref={fileRef} type="file" accept="image/*,application/pdf" capture="environment" className="hidden" onChange={handleFileSelect} />
      <canvas ref={canvasRef} className="hidden" />
    </div>
  )
}
