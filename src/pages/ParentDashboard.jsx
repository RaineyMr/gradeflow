      ...
      ws.onmessage = (event)=>{
        try {
          ...
          if(msg.type==='transcript'&&msg.transcript_event){
            ...
            if(text?.trim()) setTranscript(prev=>[...prev,{ role:speaker_type==='user'?'You':'Spark', text:text.trim(), time:new Date().toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'}) }])
          }
        } catch {}
      }
    ...
    <ModalShell onClose={onClose}>
      ...
      <div style={{ textAlign:'center', padding:'16px 0 20px' }}>
        {status==='idle'&&<>
          ...
          <div style={{ fontSize:13, color:T.muted, marginBottom:20 }}>Ask how to support {childName}'s learning at home</div>
        ...
        {status==='connecting'&&<><div style={{ fontSize:36, marginBottom:12 }}>{'\u23f3'}</div><div style={{ fontSize:13, color:T.muted }}>Connecting...</div></>}
        {status==='active'&&<>
...
function MessagesPage({ onBack }) {
  ...
  function startThread(contact) {
    ...
    setThreads(ts=>[...ts,t]); setSelectedThread(t); setShowNewRecipient(false)
  }
      ...
      <div style={{ minHeight:'100vh', background:T.bg, color:T.text, fontFamily:"'DM Sans','Helvetica Neue',sans-serif", display:'flex', flexDirection:'column' }}>
        ...
        {!isReadOnly&&(
          <div style={{ position:'fixed', bottom:0, left:0, right:0, padding:'12px 16px max(16px,env(safe-area-inset-bottom))', background:`${T.bg}f0`, backdropFilter:'blur(16px)', borderTop:`1px solid ${T.border}`, display:'flex', gap:8, alignItems:'flex-end', zIndex:100 }}>
            <textarea value={reply} onChange={e=>setReply(e.target.value)} onKeyDown={e=>{ if(e.key==='Enter'&&!e.shiftKey){ e.preventDefault(); sendReply() }}} placeholder="Reply to teacher..." rows={1}
        ...
        <div style={{ margin:'12px 16px', background:T.card, border:`1px solid ${T.secondary}40`, borderRadius:18, padding:16 }}>
          ...
          <div style={{ borderTop:`1px solid ${T.border}`, paddingTop:14 }}>
            ...
            <div style={{ display:'flex', gap:8 }}>
              <input value={newEmail} onChange={e=>setNewEmail(e.target.value)} placeholder="Email address or URL" style={{ flex:1, background:T.inner, border:`1px solid ${T.border}`, borderRadius:10, padding:'8px 12px', color:T.text, fontSize:13, outline:'none' }}/>
        ...
        {threads.filter(t=>!t.private).map(thread=>(
          <button key={thread.id} onClick={()=>setSelectedThread(thread)}
            ...
            <div style={{ flex:1, minWidth:0 }}>
              ...
              <div style={{ fontSize:11, color:T.muted, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{thread.messages[thread.messages.length-1]?.text||'No messages'}</div>
            </div>
...
export default function ParentDashboard({ currentUser }) {
  ...
  const withNav = (node) => (
    <>
      {node}
      <SharedBottomNav role="parent" active={activeNav} onSelect={navigate} isSubPage={page!=='home'}/>
    </>
  )
