...
// Private threads for home widget preview
const PRIVATE_THREADS = [
  { id:1, name:'Ms. Johnson', role:'teacher', avatar:'👩‍🏫', subject:'Math · Private',
    unread:true, private:true,
    msgs:[
  ...
  { id:2, name:'Mr. Lee', role:'teacher', avatar:'🧑‍🔬', subject:'Science · Private',
    unread:false, private:true,
    msgs:[{ id:1, from:'Mr. Lee', me:false, text:"Hi Ms. Thompson, Marcus's Science grade has dropped. I recommend additional practice.", time:'Yesterday' }] },
]

// Student view threads (read-only)
const STUDENT_THREADS = [
  ...
  { id:11, name:'Mr. Lee', role:'teacher', avatar:'🧑‍🔬', subject:'Marcus · Science',
    unread:false, readOnly:true,
    msgs:[{ id:1, from:'Mr. Lee', me:false, text:"Science fair project due Friday. Include your hypothesis!", time:'Yesterday' }] },
]
...
// ─── THREAD VIEW (inline reply bar above nav) ─────────────────────────────────
function ThreadView({ thread, onBack, senderName }) {
  ...
  function send() {
    ...
    setMsgs(m=>[...m, { id:Date.now(), from:senderName||'Me', me:true, text:reply.trim(), time:'Just now' }])
    setReply('')
  }
  ...
  return (
    <div style={{ minHeight:'100vh', background:T.bg, color:T.text, fontFamily:"'DM Sans','Helvetica Neue',sans-serif", display:'flex', flexDirection:'column' }}>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:11 }}>
          ...
          <div>
            ...
            <div style={{ display:'flex', gap:5, alignItems:'center', marginTop:2 }}>
              ...
              {thread.private  && <span style={{ fontSize:9, background:'rgba(240,74,74,0.22)', color:T.red, borderRadius:999, padding:'2px 6px', fontWeight:700 }}>🔒 Private</span>}
      ...
      {isRO && (
        <div style={{ margin:'10px 16px 0', background:'#0e0e1e', border:'1px solid rgba(255,255,255,0.08)', borderRadius:9, padding:'7px 12px', fontSize:11, color:T.muted, textAlign:'center' }}>
          👁 Viewing Marcus's messages with his teacher — read only
        </div>
      )}
        ...
        {msgs.map(m=>(
          <div key={m.id} style={{ display:'flex', justifyContent:m.me?'flex-end':'flex-start', marginBottom:13 }}>
            ...
            <div style={{ maxWidth:'78%' }}>
              ...
              <div style={{ background:m.me?T.primary:T.inner, color:m.me?'#fff':T.text, borderRadius:m.me?'15px 15px 4px 15px':'15px 15px 15px 4px', padding:'10px 13px', fontSize:13, lineHeight:1.6 }}>
                {m.text}
              </div>
      ...
      {/* Reply bar — sits above bottom nav (bottom: 72px) */}
      {!isRO && (
...
// ─── FULL MESSAGES WIDGET (on home dash) ──────────────────────────────────────
// Shows Student View / Private toggle, thread list with Reply→, opens full page on "See all"
function MessagesWidget({ navigate, openThread }) {
  const [pView, setPView] = useState('private')
  const displayList = pView==='student' ? STUDENT_THREADS : PRIVATE_THREADS
  const unread = PRIVATE_THREADS.filter(t=>t.unread).length

  return (
    <Widget style={{ background:T.card, border:`1px solid ${T.border}` }}>
      {/* Header */}
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:12 }}>
        <div>
          <div style={{ fontSize:13, fontWeight:800, color:T.text }}>💬 Messages</div>
          {unread>0 && <div style={{ fontSize:10, color:T.red, marginTop:1 }}>{unread} new message{unread>1?'s':''}</div>}
        </div>
        <div style={{ display:'flex', gap:6, alignItems:'center' }}>
          <button onClick={e=>{ e.stopPropagation(); navigate('messages') }}
            style={{ background:`${T.secondary}22`, color:T.secondary, border:`1px solid ${T.secondary}40`, borderRadius:9, padding:'5px 10px', fontSize:10, fontWeight:700, cursor:'pointer' }}>
            See all →
          </button>
        </div>
      </div>

      {/* Student View / Private toggle — unique to parent */}
      <div style={{ background:T.inner, borderRadius:12, padding:3, display:'flex', marginBottom:10 }}>
        <button onClick={e=>{ e.stopPropagation(); setPView('student') }}
          style={{ flex:1, padding:'7px', borderRadius:9, border:'none', cursor:'pointer', fontSize:11, fontWeight:700, transition:'all 0.18s',
            background:pView==='student'?'linear-gradient(135deg,#0f766e,#1d4ed8)':'transparent',
            color:pView==='student'?'#fff':T.muted }}>
          👁 Student View
        </button>
        <button onClick={e=>{ e.stopPropagation(); setPView('private') }}
          style={{ flex:1, padding:'7px', borderRadius:9, border:'none', cursor:'pointer', fontSize:11, fontWeight:700, transition:'all 0.18s',
            background:pView==='private'?'linear-gradient(135deg,#7c1d1d,#1d2040)':'transparent',
            color:pView==='private'?T.red:T.muted }}>
          🔒 Private w/ Teacher
        </button>
      </div>
      ...
      {/* Banner */}
      {pView==='private' && (
        <div style={{ background:'#180808', border:`1px solid ${T.red}28`, borderRadius:8, padding:'5px 10px', marginBottom:10, textAlign:'center' }}>
          <span style={{ fontSize:10, color:T.red, fontWeight:700 }}>🔒 PRIVATE — Only you and the teacher see these</span>
        </div>
      )}
      {pView==='student' && (
        <div style={{ background:'#080814', border:'1px solid rgba(255,255,255,0.07)', borderRadius:8, padding:'5px 10px', marginBottom:10, textAlign:'center' }}>
          <span style={{ fontSize:10, color:T.muted }}>👁 Viewing Marcus's messages with teachers — read only</span>
        </div>
      )}
      ...
      {displayList.map(t=>{
        ...
        return (
          <div key={t.id}
            style={{ background:t.unread?T.raised:T.inner, border:`1px solid ${t.unread?T.secondary+'40':T.border}`, borderRadius:13, padding:'10px 12px', marginBottom:8, display:'flex', gap:11, alignItems:'flex-start' }}
            ...
            <div style={{ flex:1, minWidth:0 }} onClick={()=>openThread(t)}>
              <div style={{ display:'flex', alignItems:'center', gap:5, marginBottom:2 }}>
                ...
                {t.private  && <span style={{ fontSize:9, color:T.red   }}>🔒</span>}
                {t.readOnly && <span style={{ fontSize:9, color:T.muted }}>👁</span>}
              </div>
            ...
            <button onClick={e=>{ e.stopPropagation(); openThread(t) }}
              style={{ background:t.readOnly?T.inner:`${T.secondary}22`, color:t.readOnly?T.muted:T.secondary, border:`1px solid ${t.readOnly?T.border:T.secondary+'40'}`, borderRadius:9, padding:'6px 10px', fontSize:10, fontWeight:700, cursor:'pointer', whiteSpace:'nowrap', flexShrink:0 }}>
        ...
        {CHILD.classes.map(c=>(
          <div key={c.id} style={{ background:T.card, border:`1px solid ${T.border}`, borderLeft:`4px solid ${c.color}`, borderRadius:16, padding:'14px 16px', marginBottom:10 }}>
            ...
            <div style={{ height:4, background:T.raised, borderRadius:2, overflow:'hidden' }}>
              <div style={{ height:'100%', width:`${c.grade}%`, background:gradeColor(c.grade), borderRadius:2 }}/>
            </div>
          </div>
        ))}
        {/* Assignments */}
...
// ─── HOME PAGE ────────────────────────────────────────────────────────────────
function HomePage({ navigate, openThread, childName }) {
  const unreadCount  = PRIVATE_THREADS.filter(t=>t.unread).length
  ...
  return (
    <div style={{ padding:'12px 12px 0' }}>
      ...
      {CHILD.alerts.length>0 && (
        <Widget onClick={()=>navigate('alerts')} style={{ border:`1px solid ${T.red}25` }}>
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:10 }}>
            <div style={{ fontSize:13, fontWeight:700, color:T.text }}>🔔 Alerts</div>
      ...
      <Widget onClick={()=>navigate('grades')}>
        <div style={{ fontSize:13, fontWeight:700, color:T.text, marginBottom:12 }}>📚 {childName}'s Classes</div>
      ...
      <Widget onClick={()=>navigate('alerts')} style={{ border:`1px solid ${T.red}20` }}>
        <div style={{ fontSize:13, fontWeight:700, color:T.text, marginBottom:6 }}>⚑ Needs Attention</div>
        <div style={{ fontSize:12, color:T.red, marginBottom:4 }}>Science 61% · Study tips available for {childName}</div>
        <button onClick={e=>{ e.stopPropagation(); navigate('alerts') }}
          style={{ background:`${T.amber}18`, color:T.amber, border:`1px solid ${T.amber}30`, borderRadius:9, padding:'5px 11px', fontSize:11, fontWeight:700, cursor:'pointer' }}>
          View →
        </button>
      </Widget>

      {/* W6: Full Messages widget with toggle */}
      <MessagesWidget navigate={navigate} openThread={openThread}/>

      {/* W7: Class Feed preview */}
      <Widget onClick={()=>navigate('feed')} style={{ background:T.card }}>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:10 }}>
          <div style={{ fontSize:13, fontWeight:700, color:T.text }}>📢 Class Feed</div>
          <span style={{ fontSize:10, color:T.muted }}>Parent can react + respond</span>
        </div>
        <div style={{ background:T.inner, borderRadius:12, padding:'10px 12px' }}>
          <div style={{ fontSize:11, fontWeight:700, color:T.text, marginBottom:3 }}>📅 Test Friday — Ch. 4 & 5!</div>
          <div style={{ fontSize:10, color:T.muted, marginBottom:6 }}>Ms. Johnson · 1hr ago · Read: 18/24</div>
          <div style={{ display:'flex', gap:10 }}>
            <span style={{ fontSize:10, color:T.muted }}>👍 12</span>
            <span style={{ fontSize:10, color:T.muted }}>★ 5</span>
            <span style={{ fontSize:10, color:T.muted }}>😊 2</span>
          </div>
        </div>
      </Widget>
      ...
      <Widget style={{ background:'linear-gradient(135deg,#0d1a3a 0%,#000d1f 100%)', border:`1px solid ${T.purple}30` }}>
        ...
        <button style={{ marginTop:8, background:'none', border:'none', color:T.teal, fontSize:11, fontWeight:600, cursor:'pointer', padding:0 }}>
...
export default function ParentDashboard({ currentUser }) {
  ...
  if (activeThread) {
    return (
      <>
        <ThreadView thread={activeThread} onBack={()=>setActiveThread(null)} senderName={parentName}/>
  ...
  if(page==='messages') return <><ParentMessages onBack={goBack} viewerRole="parent"/><BottomNav active='messages' onSelect={navSelect} isSubPage={isSubPage}/></>
  if(page==='alerts')   return <><AlertsPage   onBack={goBack}/><BottomNav active={activeNav}  onSelect={navSelect} isSubPage={isSubPage}/></>
  if(page==='feed')     return <><ClassFeed    onBack={goBack} viewerRole="parent"/><BottomNav active='feed'     onSelect={navSelect} isSubPage={isSubPage}/></>
  ...
  return (
    <div style={{ minHeight:'100vh', background:T.bg, color:T.text, fontFamily:"'DM Sans','Helvetica Neue',sans-serif", paddingBottom:90 }}>
      ...
      <HomePage navigate={navigate} openThread={setActiveThread} childName={childName}/>
