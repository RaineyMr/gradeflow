...
function LessonCalendarModal({ onClose, onSelectDay }) {
  ...
  // Generate week days (Mon to Sun of current week)
  ...
  // Simulate having lessons on certain days
  ...
  return (
    <Popup onClose={onClose} title="📅 Lesson Calendar">
      <div className="p-4">
        ...
        <div className="flex gap-2 mb-4">
          {['day', 'week', 'month'].map(v => (
            <button key={v} onClick={() => setCalView(v)}
              ...
              style={{
                background: calView === v ? 'var(--school-color)' : '#1e2231',
                color: calView === v ? 'white' : '#6b7494'
              }}>
              {v}
            </button>
          ))}
        ...
        {calView === 'day' && (
          <div className="space-y-2">
            <p className="tag-label mb-3">Today — {today.toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric' })}</p>
            {lessons.map(l => (
              <button key={l.id} onClick={() => { onSelectDay(0); onClose() }}
                ...
                <p className="font-bold text-sm text-text-primary">{l.title}</p>
                <p className="text-text-muted" style={{ fontSize: '10px' }}>{l.period} Period · {l.subject} · {l.duration} min</p>
              </button>
            ))}
          </div>
        )}
        ...
        {calView === 'week' && (
          <div>
            <p className="tag-label mb-3">{monthNames[today.getMonth()]} {today.getFullYear()}</p>
            <div className="grid grid-cols-7 gap-1 mb-2">
              {dayNames.map(d => (
                <div key={d} className="text-center" style={{ fontSize: '9px', color: '#6b7494', fontWeight: 700 }}>{d}</div>
              ))}
            </div>
            <div className="grid grid-cols-7 gap-1">
              {weekDays.map((day, i) => {
                ...
                const offset = getDayOffset(new Date(day))
                return (
                  <button key={i} onClick={() => { onSelectDay(offset); onClose() }}
                    ...
                    style={{
                      ...
                      border: hasLesson && !isToday ? '1px solid #0fb8a040' : '1px solid transparent'
                    }}>
                    <span className="font-bold text-sm" style={{ color: isToday ? 'white' : '#eef0f8' }}>{day.getDate()}</span>
                    {hasLesson && <div className="w-1 h-1 rounded-full mt-0.5" style={{ background: isToday ? 'white' : '#0fb8a0' }} />}
                  </button>
                )
              })}
            </div>
            <p className="text-text-muted mt-3 text-center" style={{ fontSize: '10px' }}>● = lesson planned</p>
          </div>
        )}
              ...
              {monthDays.map((day, i) => {
                ...
                return (
                  <button key={i} onClick={() => { onSelectDay(offset); onClose() }}
                    ...
                    <span style={{ color: isToday ? 'white' : '#eef0f8', fontWeight: isToday ? 700 : 400 }}>{day.getDate()}</span>
                    {hasLesson && <div className="w-1 h-1 rounded-full" style={{ background: isToday ? 'white' : '#0fb8a0' }} />}
                  </button>
                )
              })}
            </div>
...
function TodaysLessons() {
  ...
  // Get the display date based on offset
  const displayDate = new Date()
  displayDate.setDate(displayDate.getDate() + dayOffset)
  const isToday = dayOffset === 0
  const dayLabel = isToday ? "Today's Lessons" : dayOffset === -1 ? "Yesterday's Lessons" : dayOffset === 1 ? "Tomorrow's Lessons" : displayDate.toLocaleDateString('en-US', { weekday: 'long', month: 'short', day: 'numeric' })

  // For non-today days, generate mock lessons to demo navigation
  const mockLessonsByOffset = {
    '-1': [{ id: 'prev1', period: '1st', subject: 'Math', title: 'Ch.3 · Multiplication Review', pages: '70-83', duration: 45 }],
    '0': lessons,
    '1': [
      { id: 'next1', period: '1st', subject: 'Math', title: 'Ch.5 · Problem Solving', pages: '92-99', duration: 45 },
      { id: 'next2', period: '2nd', subject: 'Reading', title: 'Ch.8 · Main Idea & Details', pages: '116-128', duration: 50 },
    ],
    '2': [{ id: 'day2', period: '3rd', subject: 'Science', title: 'Lab: Ecosystems Introduction', pages: '44-51', duration: 55 }],
  }
  ...
  function handleTouchEnd(e) {
    ...
    if (Math.abs(deltaX) > 50) {
      if (deltaX < 0) setDayOffset(d => d + 1) // swipe left → next day
      else setDayOffset(d => d - 1)              // swipe right → prev day
      setDoneIndex(0)
    }
  ...
  return (
    <>
      ...
      >
        {/* Header row: label + calendar icon + desktop arrows */}
        <div className="flex items-center justify-between mb-2">
          ...
          <div className="flex items-center gap-1" onClick={e => e.stopPropagation()}>
            ...
            <button
              ...
              onClick={e => { e.stopPropagation(); setDayOffset(d => d - 1); setDoneIndex(0) }}
              title="Previous day"
            >‹</button>
            <button
              className="hidden md:flex w-7 h-7 items-center justify-center rounded-full transition-colors hover:bg-elevated text-text-muted"
        ...
        {/* Day navigation dots (mobile swipe hint) */}
        <div className="flex items-center gap-1 mb-2 md:hidden">
          {[-1, 0, 1, 2].map(offset => (
            <div key={offset} className="w-1.5 h-1.5 rounded-full transition-all"
              style={{ background: dayOffset === offset ? '#0fb8a0' : '#1e3a2a' }} />
          ))}
          <span className="text-text-muted ml-1" style={{ fontSize: '9px' }}>swipe to navigate</span>
        </div>
        ...
        {current ? (
          <>
            ...
            <div className="flex gap-2" onClick={e => e.stopPropagation()}>
              <button onClick={() => { setDoneIndex(i => Math.min(i + 1, dayLessons.length)); }}
              ...
              <button onClick={openLessonPlan}
                className="px-4 py-1.5 rounded-pill text-xs font-bold transition-all hover:opacity-90"
                style={{ background: '#3b7ef422', color: '#3b7ef4', border: '1px solid #3b7ef440' }}>
                View Plan →
              </button>
            </div>
            {doneIndex < dayLessons.length - 1 && (
              <p className="text-text-muted mt-2" style={{ fontSize: '9px' }}>→ {dayLessons.length - doneIndex - 1} more lesson{dayLessons.length - doneIndex - 1 !== 1 ? 's' : ''} {isToday ? 'today' : 'this day'}</p>
            )}
          </>
        ) : (
        ...
        {/* Tap hint */}
        <p className="text-text-muted mt-2" style={{ fontSize: '9px', opacity: 0.6 }}>tap anywhere to open full lesson plan</p>
      </div>

      {showCalendar && (
        <LessonCalendarModal
          onClose={() => setShowCalendar(false)}
          onSelectDay={(offset) => { setDayOffset(offset); setDoneIndex(0) }}
        />
      )}
    </>
  )
