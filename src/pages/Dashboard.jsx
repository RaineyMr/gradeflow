  ...
  return (
    <div className="widget">
      <div className="flex items-center justify-between mb-3">
        ...
        <button onClick={() => setScreen('parentMessages')} className="text-accent text-xs font-semibold">View all →</button>
      </div>
      {pending.length === 0 ? (
...
function GradingWidget() {
  const { setScreen, weights } = useStore()
      ...
      <div className="flex gap-1.5 mb-4">
        {Object.entries(weights).map(([type, w]) => (
          <div key={type} className="flex-1 text-center">
            <div className="py-1 rounded-pill text-xs font-bold mb-1" style={{ background: '#1e2231', color: '#6b7494', fontSize: '9px' }}>
