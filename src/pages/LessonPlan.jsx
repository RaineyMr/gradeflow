...
function BuildFromScratch({ onBack }) {
  ...
  function handleSave() {
    saveLessonPlan({ ...sections, type: 'manual' })
    setSaved(true)
  }
  ...
  if (saved) return (
    <div className="py-8">
      <div className="text-center mb-6">
        ...
        <p className="font-bold text-text-primary text-lg mb-1">Lesson Plan Saved</p>
        <p className="text-text-muted text-sm">"{sections.title}" is ready to share or export</p>
      </div>
      <div className="p-4 rounded-card mb-4" style={{ background: '#0f1a0a', border: '1px solid #22c97a30' }}>
        <p className="tag-label mb-3">Share To</p>
        <div className="grid grid-cols-2 gap-2 mb-3">
          {[
            { label: '📢 Class Feed', action: () => setScreen('classFeed') },
            { label: '👤 Individual Students', action: () => setScreen('gradebook') },
            { label: '🌐 All Classes', action: () => setScreen('classFeed') },
            { label: '📋 Specific Class', action: () => setScreen('classFeed') },
          ].map(s => (
            <button key={s.label} onClick={s.action}
              className="py-2 rounded-pill text-xs font-semibold"
        ...
        <div className="flex gap-2">
          <button onClick={() => window.print()} className="flex-1 py-2 rounded-pill text-xs font-bold" style={{ background: '#3b7ef420', color: '#3b7ef4' }}>⬇ PDF</button>
          <button onClick={() => window.print()} className="flex-1 py-2 rounded-pill text-xs font-bold" style={{ background: '#9b6ef520', color: '#9b6ef5' }}>🖨 Print</button>
          <button onClick={() => navigator.clipboard?.writeText(JSON.stringify(sections))} className="flex-1 py-2 rounded-pill text-xs font-bold" style={{ background: '#f5a62320', color: '#f5a623' }}>📋 Copy</button>
        </div>
      </div>
      <button onClick={onBack} className="w-full py-2.5 rounded-pill text-sm font-semibold" style={{ background: '#1e2231', color: '#6b7494' }}>
