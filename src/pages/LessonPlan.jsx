  ...
  if (done) return (
    <div>
      ...
      <div className="flex gap-2">
        <button className="flex-1 py-2.5 rounded-pill text-sm font-bold" style={{ background: 'var(--school-color)', color: 'white' }}>
          📤 Share to Class
        </button>
    ...
    <div>
      ...
      <div className="p-3 rounded-card mb-3" style={{ background: '#0f1a0a', border: '1px solid #22c97a30' }}>
        ...
        <div className="flex flex-wrap gap-2">
          {['Class Feed', 'All Classes', 'Specific Class', 'Individual Students'].map(s => (
            <button key={s} className="px-3 py-1.5 rounded-pill text-xs font-semibold" style={{ background: '#22c97a20', color: '#22c97a' }}>{s}</button>
          ))}
        </div>
      </div>

      <div className="flex gap-2">
        {[{ icon: '⬇', label: 'PDF', color: '#3b7ef4' }, { icon: '📋', label: 'Copy', color: '#f5a623' }, { icon: '🖨', label: 'Print', color: '#9b6ef5' }].map(e => (
          <button key={e.label} onClick={() => e.label === 'Print' ? window.print() : e.label === 'Copy' ? navigator.clipboard?.writeText(JSON.stringify(plan)) : null}
