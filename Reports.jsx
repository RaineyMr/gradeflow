    ...
    <div>
      ...
      <div className="p-4 rounded-card" style={{ background: '#1e2231' }}>
        <div className="flex items-center gap-3">
          ...
          {[{ icon: '🖨', label: 'Print' }, { icon: '⬇', label: 'PDF' }, { icon: '📋', label: 'Spreadsheet' }].map(e => (
            <button key={e.label} className="px-3 py-1.5 rounded-pill text-xs font-semibold" style={{ background: '#161923', color: '#6b7494' }}>
              {e.icon} {e.label}
            </button>
          ))}
        </div>
